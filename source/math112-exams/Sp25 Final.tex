...
\end{tikzpicture}
\begin{checkboxes}
    \choice \hspace{-3em} A. \hspace{1.2em} $f(x)=2|x-1|+2$
    \choice \hspace{-3em} B. \hspace{1.2em} $f(x)=2|x+1|+2$
    \choice \hspace{-3em} C. \hspace{1.2em} $f(x)=\frac{1}{2}|x-1|+2$
    \choice \hspace{-3em} D. \hspace{1.2em} $f(x)=\frac{1}{2}|x+1|+2$
...
% \question[3] Consider the following system of equations:
% $$\begin{cases}
% \phantom{-} x  - 2 y +Az& = 3 \\
%  2x  -  4 y +10z& = 6\\
% -3x+6y-15z&=-9
% \end{cases}$$
% What is $A$ if there is an infinite plane of solutions to the system?
% \begin{choices}
%    \choice \chooseone $A=5$
%    \choice \chooseone $A=-5$
%    \choice \chooseone $A=3$
%    \choice \chooseone $A=-3$
%    \choice \chooseone None of the above.
% \end{choices}
...
$
            ...
            \end{center}
\newpage 
% \question[5] For the following system of linear equations, determine whether or not the system has any solutions. If the system has a solution, state it as $(x,y,z)$; if the systems has infinitely many solutions, write your solutions in terms of the variable $x$ only.
% \[
% \left\{\begin{matrix}
%  x  &+& 2 y & &&=& 1 \\
% - x & & &+ & 3 z & =& 13\\
